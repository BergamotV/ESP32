...
static esp_err_t event_handler(void *ctx, system_event_t *event)
{
    ...
    case SYSTEM_EVENT_AP_STACONNECTED:
    	...
    	xEventGroupSetBits(udp_event_group, WIFI_CONNECTED_BIT);
       // 複数接続があり得るので、WIFI_CONNECTED_BIT での管理はなくす方針
    	break;
...
void send_data(void *pvParameters)
{
    ...
    int i2s_read_len = EXAMPLE_DEFAULT_PKTSIZE;
    ...
    while(1)
    {
        read_i2s_adc( i2s_write_buff, i2s_read_len, &bytes_read );
...
void recv_data(void *pvParameters)
{
	...
	if (len > 0)
       {
           // UDPの場合、パケットは結合される事なく来るので、１つのデータを受信したと見做して処理する。
	    total_data += len;
	    success_pack++;
	}
       else
